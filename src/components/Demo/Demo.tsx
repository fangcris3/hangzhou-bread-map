import { loadStores } from '../../data/storage'
        ...
        return (
          <section key={id} className="px-8 py-12">
            ...
            <div className={`grid grid-cols-1 ${gridCols} border-l border-t border-dashed border-ink/15`}>
              {stores.map(store => (
                <div key={store.id} className="border-r border-b border-dashed border-ink/15">
                  <Card store={store} />
                </div>
              ))}
              {/* fill empty grid cells for visual balance */}
              {cols === 3 && stores.length < 3 && (
